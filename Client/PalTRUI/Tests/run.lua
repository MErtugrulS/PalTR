local source = debug.getinfo(1, "S").source
local path = source:sub(1, 1) == "@" and source:sub(2) or source
local directory = path:gsub("\\", "/"):match("^(.*)/") or "."

package.path = directory .. "/../Package/Scripts/?.lua;" .. package.path
package.path = directory .. "/../../../Scripts/?.lua;" .. package.path

dofile(directory .. "/view_model_spec.lua")
dofile(directory .. "/contract_spec.lua")
dofile(directory .. "/snapshot_inbox_spec.lua")
dofile(directory .. "/server_snapshot_contract_spec.lua")
dofile(directory .. "/server_ui_pipeline_spec.lua")
dofile(directory .. "/server_command_response_spec.lua")
dofile(directory .. "/action_intent_spec.lua")
dofile(directory .. "/action_outbox_spec.lua")
dofile(directory .. "/chat_command_sender_spec.lua")
dofile(directory .. "/chat_receive_probe_spec.lua")
dofile(directory .. "/presentation_controller_spec.lua")
dofile(directory .. "/presentation_snapshot_probe_spec.lua")
dofile(directory .. "/relation_navigator_spec.lua")
dofile(directory .. "/renderer_host_spec.lua")
dofile(directory .. "/umg_asset_loader_spec.lua")
dofile(directory .. "/umg_context_spec.lua")
dofile(directory .. "/umg_probe_spec.lua")
dofile(directory .. "/umg_button_state_probe_spec.lua")
dofile(directory .. "/umg_button_poller_spec.lua")
dofile(directory .. "/application_focus_guard_spec.lua")
dofile(directory .. "/umg_button_hook_poller_spec.lua")
dofile(directory .. "/umg_widget_port_spec.lua")
dofile(directory .. "/umg_view_binder_spec.lua")
dofile(directory .. "/ui_interaction_router_spec.lua")
dofile(directory .. "/ui_wire_spec.lua")
dofile(directory .. "/snapshot_codec_spec.lua")
dofile(directory .. "/snapshot_transport_spec.lua")
