...
function CommandService:_respond(player, raw, success, message)
...
function CommandService:on_chat(controller_param, message_param)
    ...
    if command.action == "HELP" then
        self:_respond(
            player, command.raw, true,
            "/paltr durum | /paltr klanlar | /savas KLAN | " ..
            "/ateskes KLAN | /ittifak KLAN | /kabul KLAN | /reddet KLAN"
        )
        return
    end
    ...
    if not allowed then
        self:_respond(player, command.raw, false, error_message)
        return
    end

    local target, target_error = self:_target(player, command.target)
    if not target then
        self:_respond(player, command.raw, false, target_error)
        return
    end
    ...
    if command.action == "DECLARE_WAR" then
        result = self.diplomacy:declare_war(
            player.guild_key, target.key, player.name
        )
    ...
    if not result then
        self:_respond(player, command.raw, false, "Komut uygulanamadi")
